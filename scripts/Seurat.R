...
patchwork::wrap_plots(scplots, ncol=8) %T>% ggsave(filename = "figures/Seurat/Seurat-integrated.pdf", width = 45, height = 25, units = "in", dpi = 300)

scplots <- purrr::map(levels(as.factor(scRNA$subclass)), function(x) SpatialFeaturePlot(U2.Seurat, x))
patchwork::wrap_plots(scplots, ncol=8) %T>% ggsave(filename = "figures/Seurat/Seurat-integrated.pdf", width = 45, height = 25, units = "in", dpi = 300)
# Some of these outputs will still be blank, so to combat this we can group cell types together.
